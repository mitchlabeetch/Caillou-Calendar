...
export function EventDetailModal({ isOpen, onClose, eventId }: { isOpen: boolean, onClose: () => void, eventId: string }) {
  ...
  // RBAC: derive edit/delete capabilities from the user role.
  // For child accounts the current user must match one of the event's
  // member IDs to be allowed to touch it.
  const memberUserId = user?.uid === 'local-user' ? undefined : user?.uid;
  const canEdit = event ? canEditEvent(userRole, event, memberUserId) : false;
  const canDelete = event ? canDeleteEvent(userRole, event, memberUserId) : false;

  const handleDownloadIcs = () => {
    if (!event) return;
    exportEventsToICS([event], familyMembers);
  };
  ...
  return (
    <ModalShell
      ...
      backdropClassName="bg-ink/20 backdrop-blur-sm"
      panelClassName="pointer-events-auto w-full sm:max-w-md h-[100dvh] sm:h-auto sm:max-h-[90vh] overflow-hidden flex flex-col bg-surface border-[4px] border-ink sm:rounded-3xl rounded-none sm:shadow-neo-xl"
      showCloseButton={!event.thumbnailUrl}
    >
      {event.thumbnailUrl && (
        <div className="w-full h-48 border-b-[4px] border-ink relative shrink-0">
          ...
          <button
            ...
            className="absolute top-4 right-4 w-10 h-10 bg-surface border-[3px] border-ink rounded-full flex items-center justify-center shadow-neo hover:scale-105 transition-transform z-10"
          >
