...
export function SetStatusModal({
  ...
  const panelClass = member?.bgClass ?? 'bg-surface';
  ...
  return (
    <ModalShell
      ...
      onClose={onClose}
      title={t('app.setMyStatus')}
      titleClassName="text-2xl font-display font-bold"
      maxWidth="sm:max-w-md"
      wrapperClassName="p-0 sm:p-4"
      panelClassName={cn(
        "w-full h-[100dvh] sm:h-auto sm:max-h-[90vh] rounded-none sm:rounded-3xl p-6 pt-12 sm:pt-6 flex flex-col gap-4 overflow-y-auto mt-auto sm:mt-0 border-[4px] border-ink sm:shadow-neo-xl",
        panelClass,
      )}
    >
      <div className="flex flex-col overflow-y-auto overflow-x-hidden pr-2">
        <div className="flex flex-col gap-2 relative">
            ...
            <div className="relative">
              <button
                ...
                className={cn("w-full h-14 border-[3px] border-ink rounded-xl px-4 font-bold bg-surface flex items-center justify-between shadow-neo-md hover:-translate-y-0.5 hover:shadow-neo transition-all", useCustom ? "opacity-50 grayscale" : "")}
              >
              ...
              <AnimatePresence>
                {presetDropdownOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -10 }}
                    className="absolute top-[calc(100%+8px)] left-0 w-full bg-surface border-[3px] border-ink rounded-xl shadow-neo z-20 overflow-hidden"
                  >
                    {PRESETS.map(p => {
        ...
        <div className="flex flex-col gap-3">
            <div className="flex gap-2 relative mt-2">
              <div className="w-14 h-14 bg-surface border-[3px] border-ink rounded-xl flex items-center justify-center shrink-0 shadow-neo-md">
              ...
              <input
                ...
                className="flex-1 h-14 border-[3px] border-ink rounded-xl px-3 font-bold bg-surface outline-none uppercase shadow-neo-md"
              />
            </div>
